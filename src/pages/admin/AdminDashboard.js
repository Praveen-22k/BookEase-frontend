...
export default function AdminDashboard() {
  const [stats, setStats] = useState({ services: 0, bookings: 0, confirmed: 0, cancelled: 0, revenue: 0 });
  ...
  useEffect(() => {
    Promise.all([
      ...
      setStats({ services: svcRes.data.length, bookings: bookings.length, confirmed: confirmed.length, cancelled: cancelled.length, revenue });
  ...
  const statCards = [
    { label: 'Total Services', value: stats.services, icon: 'bi-grid-3x3-gap', color: '#1a3c5e', bg: '#e8f0f9' },
    { label: 'Total Bookings', value: stats.bookings, icon: 'bi-calendar-check', color: '#6f42c1', bg: '#f3eeff' },
    { label: 'Confirmed', value: stats.confirmed, icon: 'bi-check-circle', color: '#198754', bg: '#e8f5ee' },
    { label: 'Revenue', value: `₹${stats.revenue.toFixed(0)}`, icon: 'bi-currency-dollar', color: '#c9a84c', bg: '#fdf8ec' },
  ];
          ...
          <div key={i} className="col-sm-6 col-xl-3">
            <div className="stat-card d-flex align-items-center gap-3">
              ...
              <div>
                ...
                <h3 className="mb-0 fw-bold" style={{ color: card.color, fontFamily: 'DM Sans, sans-serif' }}>{card.value}</h3>
              </div>
            </div>
          </div>
        ))}
                ...
                <tbody>
                  {recentBookings.map(b => (
                    <tr key={b._id}>
                      ...
                      <td>
                        <p className="mb-0 fw-semibold" style={{ fontSize: '0.9rem' }}>{b.customer.name}</p>
