...
export default function AdminLayout() {
  ...
  return (
    <div className="d-flex">
      {/* Sidebar */}
      <div className="admin-sidebar d-flex flex-column" style={{ paddingTop: '0' }}>
        <div className="p-4 border-bottom" style={{ borderColor: 'rgba(255,255,255,0.1) !important' }}>
          <h5 className="mb-0" style={{ fontFamily: 'Playfair Display, serif', color: 'var(--accent)' }}>
            <i className="bi bi-calendar-check-fill me-2"></i>BookEase
          </h5>
          <p className="mb-0 mt-1" style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.5)' }}>Admin Panel</p>
        </div>
        ...
        <div className="p-3 border-top" style={{ borderColor: 'rgba(255,255,255,0.1)' }}>
          <div className="d-flex align-items-center gap-2 mb-2">
            <div className="rounded-circle d-flex align-items-center justify-content-center"
              style={{ width: 36, height: 36, background: 'var(--accent)', color: 'var(--primary)', fontWeight: 700 }}>
              {user?.name?.[0]?.toUpperCase()}
            </div>
            <div>
              <p className="mb-0" style={{ fontSize: '0.85rem', color: 'white', fontWeight: 600 }}>{user?.name}</p>
              <p className="mb-0" style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.5)' }}>Administrator</p>
            </div>
          </div>
